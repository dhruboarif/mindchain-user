   ...
   <div class="bd-example">
     ...
     @endif
                    <div class="card-body">
                      ...
                      @else
                        <h4 class="card-title">MIND Withdraw Report</h4>
                        @if(Auth::user()->status == 0)
                        <a class="btn btn-primary float-right disabled" href="#" data-bs-toggle="modal" data-bs-target="#withdrawbonus">Add Request</a>

                        @else
                        <a class="btn btn-primary float-right" href="#" data-bs-toggle="modal" data-bs-target="#withdrawbonus">Make Withdraw</a>
                        @include('user.modals.withdrawbonusmodal')
                        @endif
                        <hr>
                                       ...
                                       <tr>
                                          <td >{{$loop->index+1}}</td>
                                           ...
                                           <td>{{$row->amount}}</td>
                                            ...
                                            <a data-bs-toggle="modal" data-bs-target="#withdrawconfirmation{{$row->id}}" class="btn btn-danger">Confirm Withdraw</a>
                                            @elseif($row->status == 'pending')
                                            <a data-bs-toggle="modal" data-bs-target="#withdrawMindCancel{{$row->id}}" class="btn btn-danger">Cancel</a>
                                            @else
                                           {{$row->status}}
                                        @endif</td>
